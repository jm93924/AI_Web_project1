...
def get_today_strawberry_wholesale_price():
    ...
    avg_items = [
        x for x in items
        if isinstance(x, dict) and x.get("countyname") == "평균"
    ]
    ...
    def sort_key(x):
        ...
        regday = str(x.get("regday", "")).strip()   # MM/DD
    ...
    yyyy = str(latest.get("yyyy", "")).strip()
    regday = str(latest.get("regday", "")).strip()

    trade_date = None
    try:
        month, day = map(int, regday.split('/'))
        trade_date = datetime(int(yyyy), month, day)
    except Exception:
        pass
